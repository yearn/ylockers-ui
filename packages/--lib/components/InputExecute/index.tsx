...
function Provided({className, noInput = false}: {className?: string; noInput?: boolean}) {
	...
	const label = useMemo(() => {
		...
		if (approve.receipt.isLoading)
			return {
				key: 'approve-confirm',
				text: 'Confirming'
			};
		if (execute.receipt.isLoading)
			return {
				key: 'execute-confirm',
				text: 'Confirming'
			};
	...
	const subtext = useMemo(() => {
		...
		if ((!needsApproval || isApproved) && execute.receipt.isSuccess) {
			return {
				key: 'success',
				text: (
					<GreatSuccess
						hash={execute.hash!}
						message={`You ${verbPastTense} ${fTokens(amountExecuted, token.decimals)} ${token.symbol}!`}
					/>
				)
			};
		}
			...
			<div className={`flex gap-2 ${noInput ? 'justify-center' : ''}`}>
				...
				<Button
					...
					className={`shrink-0 capitalize overflow-hidden ${
						noInput ? 'text-black bg-yellow-400 hover:bg-yellow-400 hover:opacity-70' : ''
					}`}
...
export default function InputExecute({
	task,
	className,
	yDaemon,
	env
