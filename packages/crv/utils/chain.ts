...
type ChainableValue<T> = {
	...
	reduce: <U>(
		fn: (acc: U, item: T extends (infer U)[] ? U : T, index?: number) => U,
		initial: U
	) => ChainableValue<U>;
	...
	fromPairs: () => ChainableValue<Record<string, any>>;
};
...
class Chain<T> implements ChainableValue<T> {
	...
	fromPairs(): ChainableValue<Record<string, any>> {
		if (!Array.isArray(this.data)) {
			throw new Error('fromPairs can only be called on arrays');
		}
		const result: Record<string, any> = {};
		const pairs = this.data as any[];
...
const _ = {
	...
	fromPairs: <K extends PropertyKey, V>(pairs: [K, V][]): Record<K, V> => {
