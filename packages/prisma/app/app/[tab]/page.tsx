    ...
    <div className="flex flex-col">
      ...
      {props.leftActive ? (
        <Header
          ...
          launchApp={false}
          selected={tab === 'get' ? `Get ${LOCKER_TOKEN_NAME}` : tab === 'stake' ? 'Stake' : tab === 'learn_more_stake' ? 'Learn More' : tab === 'unstake' ? 'Unstake' : tab === 'claim' ? 'Claim Rewards' : ''}
          ...
          <div className="flex flex-col md:flex-row space-y-6 md:space-y-0 w-full pt-0"> 
            ...
            <div className="flex flex-col space-y-6 w-full md:w-1/2 p-4 md:p-8 pt-0">
              ...
              <span className="font-thin opacity-70">
                Stake your {LOCKER_TOKEN_NAME} and start earning a share of Yearn&apos;s ve{BASE_TOKEN_NAME} stablecoin revenue today. You&apos;ll reach max boost and hit the maximum staking APR after just 4 weeks.
              </span>
        ...
        {tab === 'unstake' && (
          <div className="flex flex-col md:flex-row space-y-6 md:space-y-0 w-full pt-0">
            ...
            <div className="flex flex-col space-y-6 w-full md:w-1/2 p-4 md:p-8 pt-0">
              ...
              <span className="font-thin opacity-70">
                Unstake your {LOCKER_TOKEN_NAME}. You&apos;re free to unstake at any time with no lock-ups or penalties. Unstaked {LOCKER_TOKEN_NAME} doesn&apos;t earn any yield. Withdrawals pull from the least boosted position.
              </span>
              <Image alt="charge multiplier" className="" src="/charge.png" width={370} height={136} />
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row space-y-6 md:space-y-0 w-full pt-0"> 
            ...
            <div className="flex flex-col space-y-4 w-full md:w-1/2 p-4 md:p-8 pt-0">
              ...
              <p className="font-thin opacity-70">
                Claim your {STABLE_TOKEN_NAME} rewards. We already deposited your {STABLE_TOKEN_NAME} into our auto-compounding {STABLE_TOKEN_NAME} vault (
                <A target="_blank" rel="noreferrer" className="underline" href={`https://yearn.fi/v3/1/${STABLE_TOKEN_VAULT}`}>{STABLE_TOKEN_VAULT_NAME}</A>
                ).
              </p>
              <p className="font-thin opacity-70">
                That means your yield has been earning you additional yield from the moment we received it. Once claimed, your {STABLE_TOKEN_NAME} vault holdings will appear below.
              </p>
              <div>
        ...
        {tab === 'get' && (
          <div className="flex">
            <div className="flex flex-col p-4 md:p-8 w-full md:w-2/3">
              <span className="font-thin pb-1 text-md">Mint {LOCKER_TOKEN_NAME} from {BASE_TOKEN_NAME}</span>
              <Mint yDaemon={YDAEMON} env={ENV} />
              <div className="mt-4 flex flex-col space-y-4">
                <p className="font-thin opacity-70">
                  Convert your {BASE_TOKEN_NAME} to {LOCKER_TOKEN_NAME} using the {LOCKER_TOKEN_NAME} contract. This mints {LOCKER_TOKEN_NAME} in a 1:1 ratio. ⚠️ Depending on peg it may be more efficient to use a DEX and swap instead of minting.
                </p>
                <p className="font-thin opacity-70">
                  <b>⚠️ Important: </b>
                  yLocker tokens (such as {LOCKER_TOKEN_NAME}) can never be redeemed for the underlying locked tokens ({BASE_TOKEN_NAME}). However, because they are liquid, they can be traded on decentralized exchanges, and bought and sold at the current market rate.
                </p>
              </div>
            </div>
          </div>
        ...
        {tab === 'learn_more_stake' && (
          <div className="flex flex-row space-y-6 w-full pt-0"> 
            <div className="flex flex-col space-y-4 p-4 md:p-8 w-full md:w-2/3">
              ...
              <p className="font-thin opacity-70">
                The longer you stake, the greater your boost! Yearn&apos;s {LOCKER_TOKEN_NAME} staking contract incentivizes long-term users by boosting their yield (up to a maximum of 2.5x). You&apos;ll reach max boost and achieve the maximum staking APR less than four weeks after depositing your {LOCKER_TOKEN_NAME}.
              </p>
              <p className="font-thin opacity-70">
          ...
          <div className="flex">
            <div className="flex flex-col p-4 md:p-8 w-full md:w-2/3">
              ...
              <span className="mt-4 font-thin opacity-70">
                Deposit your {LOCKER_TOKEN_NAME} into Yearn&apos;s auto-compounding vault and start earning the maximum APY immediately. The vault will handle staking, claiming and swapping rewards, and reinvesting your {LOCKER_TOKEN_NAME} for you.
              </span>
            </div>
          </div>
        )}
        {tab === 'withdraw' && (
          <div className="flex">
            <div className="flex flex-col p-4 md:p-8 w-full md:w-2/3">
              ...
              <span className="mt-4 font-thin opacity-70">
                Withdraw your {LOCKER_TOKEN_NAME} from Yearn&apos;s auto-compounding vault. Please note that this will unstake your {LOCKER_TOKEN_NAME} (and unstaked {LOCKER_TOKEN_NAME} doesn&apos;t earn any yield).
              </span>
            </div>
          </div>
        )}
