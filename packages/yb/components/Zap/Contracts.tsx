...
export default function Contracts({children}: {children: ReactNode}) {
	...
	const {inputAmountExpanded, inputIsYbs, outputIsYbs} = useParameters();
	...
	const needsYbsApproval = useMemo(() => {
		...
		return false;
	}, [inputIsYbs, outputIsYbs, approveYbsAsInput, approveYbsAsOutput]);
	...
	const isConfirming = useMemo(() => {
		return (
			approveErc20.confirmation.isFetching ||
