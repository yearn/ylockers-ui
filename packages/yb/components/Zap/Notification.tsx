...
export default function Notification({className}: {className?: string}) {
	...
	const {inputToken, inputAmount, outputToken, outputAmount} = useParameters();
	...
	const zapped = useMemo(() => {
		...
		return `Zapped ${locale.format(amountIn)} ${inputToken.symbol} → ${locale.format(amountOut)} ${
			outputToken.symbol
		}`;
	}, [inputToken, inputAmount, outputToken, outputAmount]);

	const {approveErc20, approveYbsAsInput, approveYbsAsOutput, zap, needsApproval, isVerifying, isConfirming} =
		useContracts();
	...
	useEffect(() => {
		if (isVerifying) {
			...
			setInfo(
				makeNotificationWithExplorerLink({
	...
	useEffect(() => {
		if (approveErc20.simulation.isError || approveErc20.confirmation.isError) {
			...
			console.error(
				approveYbsAsOutput.simulation.error?.message ?? approveYbsAsOutput.confirmation.error?.message
			);
			setError({
