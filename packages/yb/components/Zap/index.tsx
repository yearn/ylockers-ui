...
function Layout() {
	...
	return (
		<div className="w-full sm:w-[32rem] sm:p-4 flex flex-col gap-1">
			<div className="h-[22.5rem]">
				{selectTokenMode && (
					<motion.div
						key={`select-token-${selectTokenMode}`}
						transition={springs.rollin}
						initial={mounted ? {y: 10} : false}
						animate={{y: 0}}>
						<Suspense fallback={<></>}>
							<SelectToken
								mode={selectTokenMode}
								onClose={() => setSelectTokenMode(undefined)}
							/>
						</Suspense>
					</motion.div>
				)}
        ...
        bg-transparent rounded-xl`}>
						<InputPanel
							mode="in"
							onSelectToken={() => setSelectTokenMode('in')}
						/>
						<InputPanel
							mode="out"
							onSelectToken={() => setSelectTokenMode('out')}
						/>
						<div
							className={`pt-1
